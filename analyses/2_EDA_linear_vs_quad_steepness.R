...
lmfits<-function(.data,N,s){
  ...
  lfit <- lm(steep~ks_pct, data=bugset_fits)


  fname<-glue::glue("steep_linfit_{sqrt(N)}x{sqrt(N)}_{s}_spacing_lineplot.png")
  png(here::here("output","diagnostic",fname), width=800,height=800,units="px")
  title<-glue::glue("Diagnostic lm(steep~ks_pct) {sqrt(N)}x{sqrt(N)} {s} spacing")
  plot(x=bugset_fits$ks_pct,y=bugset_fits$steep,pch=16,col='blue',main=title)
  abline(lfit)
  dev.off()

  fname<-glue::glue("steep_linfit_{sqrt(N)}x{sqrt(N)}_{s}_spacing_diag.png")
  png(here::here("output","diagnostic",fname), width=800,height=800,units="px")
  par(mfrow=c(2,2))
  title<-glue::glue("Diagnostic lm(steep~ks_pct) {sqrt(N)}x{sqrt(N)} {s} spacing")
